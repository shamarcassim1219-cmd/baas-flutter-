...
class _JobRequestsScreenState extends State<JobRequestsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _reject(BaasOrder order) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Reject the request for "${order.service}"?'),
        actions: [
  ...
  Future<void> _complete(BaasOrder order) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Mark "${order.service}" as complete? Do this once the job is actually finished.'),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Cancel')),
          TextButton(onPressed: () => Navigator.of(context).pop(true), child: const Text('Mark Complete')),
        ],
      ),
    );
        ...
        child: _loading
            ...
            : RefreshIndicator(
                ...
                child: TabBarView(
                  controller: _tabController,
                  children: [
                    _buildList(_incoming, _buildIncomingCard),
                    _buildList(_active, _buildActiveCard),
                    _buildList(_completed, _buildCompletedCard),
                  ],
                ),
              ),
      ),
  ...
  Widget _buildList(List<BaasOrder> orders, Widget Function(BaasOrder) cardBuilder) {
    if (orders.isEmpty) {
      return ListView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (order.distanceKm != null) ...[
            ...
            Text('${order.distanceKm!.toStringAsFixed(1)} km away', style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
          ],
          const SizedBox(height: 10),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('${order.days} day${order.days == 1 ? '' : 's'}', style: const TextStyle(fontSize: 12.5, color: AppColors.textMuted)),
      ...
      child: Column(
        ...
        children: [
          ...
          if (order.customerName != null) ...[
            const SizedBox(height: 6),
            Text('Customer: ${order.customerName}', style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600)),
          ],
