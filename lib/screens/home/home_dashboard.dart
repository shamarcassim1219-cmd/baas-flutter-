...
class HomeDashboardState extends State<HomeDashboard> with WidgetsBindingObserver {
  ...
  /// Refreshes online/offline status, platform fee, and earnings
  /// whenever the app comes back to the foreground - without this,
  /// re-opening the app after it auto-went-offline at midnight (or
  /// after a fee/earnings change from another device) would keep
  /// showing whatever was on screen when it was last backgrounded.
  @override
  ...
  /// Called from HomeShell when switching back to this tab, so an
  /// online/offline change made elsewhere (e.g. accepting a job,
  /// which takes the Baas offline automatically) shows up here
  /// right away instead of only on the next app resume.
  ...
  bool get _isVerified => (_user?['verification'] as Map<String, dynamic>?)?['status'] == 'Approved';
  ...
  void _showVerificationRequiredDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'You need to complete identity verification and be approved before you can go online.',
        ),
        actions: [
  ...
  Future<void> _toggleOnline(bool value) async {
    // Going online requires an Approved identity verification - a
    // Pending/Rejected/never-submitted Baas is blocked client-side
    // here so they get a clear explanation, rather than a raw
    // backend error the first time they try.
