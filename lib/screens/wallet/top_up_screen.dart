...
class _TopUpScreenState extends State<TopUpScreen> {
  ...
  Map<String, dynamic> _mapToPayHerePayment(Map<String, dynamic> p) {
    return {
      ...
      'order_id': p['orderId'],
      'items': _isFee ? 'GOBAAS Platform Fee' : 'GOBAAS Wallet Top-up',
      'amount': p['amount'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(_isFee ? t('payPlatformFee') : t('topUp'))),
      body: SafeArea(
        child: ListView(
          padding: const EdgeInsets.all(20),
          children: [
            Text(_isFee ? t('amountDue') : t('amountLkr'), style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 4),
            Text(
              _isFee ? t('fixedFeeNote') : t('minimumTopUp'),
